...
let getTrailScore (map: int array array) (x: int) (y: int) =
    ...
    findTrails x y 0 |> List.distinct |> List.length

let allCoordinates (twoD: 'a array array) =
    [0..twoD.Length-1] |> Seq.collect (fun y -> [0..twoD[y].Length-1] |> Seq.map (fun x -> (x, y)))

let scores = allCoordinates input |> Seq.map (fun (x, y) -> getTrailScore input x y) |> Seq.toList
let sum = scores |> List.sum
printf $"Sum of scores is {sum}"